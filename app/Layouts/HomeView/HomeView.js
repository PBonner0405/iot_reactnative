        ...
        setInterval(()=>{
            ...
            request.onreadystatechange = (e) => {
                ...
                if (request.status === 200) {
                    ...
                    if(Math.abs(parseFloat(this.state.T1) - parseFloat(this.state.T2)) > parseFloat(this.state.D_T)){
                        PushNotification.localNotificationSchedule({
                            //... You can use all the options from localNotifications
                            message: this.state.notification_1, // (required)
                            date: new Date(Date.now() + 10), // within 0.01sec
                            vibrate: true, // (optional) default: true
                            vibration: 500, // vibration length in milliseconds, ignored if vibrate=false, default: 1000
                            title: "Difference Warning", // (optional)
                        });
                    }
                    if((parseFloat(this.state.T1) < parseFloat(this.state.T_L1)) || parseFloat(this.state.T1) > parseFloat(this.state.T_H1)
                        ||(parseFloat(this.state.T2) < parseFloat(this.state.T_L2)) || parseFloat(this.state.T2) > parseFloat(this.state.T_H2)){
                        PushNotification.localNotificationSchedule({
                            //... You can use all the options from localNotifications
                            message: this.state.notification_2, // (required)
                            date: new Date(Date.now() + 10), // within 0.01sec
                            vibrate: true, // (optional) default: true
                            vibration: 500, // vibration length in milliseconds, ignored if vibrate=false, default: 1000
                            title: "Out of Range Warning", // (optional)
                        });
                    }
                    this.setState({
                        T1: response.fsr01,
